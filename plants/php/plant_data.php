...
if(isset($_REQUEST['start']) && isset($_REQUEST['end']) && $_REQUEST['start'] != "" && $_REQUEST['end'] == "")
	$dateCondition = "IA.schedule_date >= '".$_REQUEST['start']."'";
else
if(isset($_REQUEST['start']) && isset($_REQUEST['end']) && $_REQUEST['start'] == "" && $_REQUEST['end'] != "")
	$dateCondition = "IA.schedule_date <= '".$_REQUEST['end']."'";
else
if(isset($_REQUEST['start']) && isset($_REQUEST['end']) && $_REQUEST['start'] != ""&& $_REQUEST['end'] != "")
	$dateCondition = "IA.schedule_date >= '".$_REQUEST['start']."' AND IA.schedule_date <= '".$_REQUEST['end']."'";
else
...
$query = "SELECT IA.id, I.qty, IA.status, I.plant, B.id AS block_id, S.name AS size, B.name AS block, IA.inventory_id, IA.job_id , schedule_date as date, J.name as job_name, J.type, J.ref, IA.ref_id, IA.material_cost, IA.completed, L.name AS labor_name, L.cost AS labor_cost, labor_id, IA.employee_id, E.full_name AS employee, IA.completed_date, IA.ticket    
			...
			ON IA.employee_id = E.id LEFT JOIN sizes S 
			ON I.size = S.id 
			WHERE $completedCondition AND $dateCondition AND $blockCondition AND $sizeCondition AND $jobCondition AND $plantCondition AND $employeeCondition $materialCondition AND $nameCondition ";
