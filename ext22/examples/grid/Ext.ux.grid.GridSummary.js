...
Ext.extend(Ext.ux.grid.GridSummary, Ext.util.Observable, {
  init : function(grid) 
  {
	...
	grid.on('afteredit', this.refreshSummary, this);
	...
	grid.store.on('update', this.refreshSummary, this);
	grid.store.on('scope', this.refreshSummary, this);
...
Ext.ux.grid.GridSummary.Calculations = {
  'sum' : function(v, record, field) 
  {
	...
	return parseFloat(v) + (record.data[field] != '' ? parseFloat(record.data[field]) : 0);
  },
